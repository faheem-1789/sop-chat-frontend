...
const SummarizeIcon = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>;
...
    const handleUpload = async (filesToUpload, isMoreUpload = false) => {
        ...
        try {
            ...
            newMessages.push({ role: 'assistant', text: `Successfully processed ${fileNames}.` });

            if (summary) {
                newMessages.push({ role: 'assistant', text: `**Proactive Summary of your document(s):**\n\n${summary}` });
            }
            ...
            setToast({ show: true, message: `An error occurred during upload: ${error.response?.data?.detail || error.message}`, type: 'error' });
        } finally {
            loadingSetter(false);
...
    };
    ...
    const handleSummarize = async () => {
        if (!user || chat.length < 2) {
            setToast({ show: true, message: 'Not enough conversation to summarize.', type: 'error' });
            return;
        }
        setIsSummarizing(true);
        try {
            const token = await getIdToken(user);
            const historyPayload = chat.reduce((acc, curr, i, arr) => {
                if (curr.role === 'user' && arr[i + 1]?.role === 'assistant') {
                    acc.push([curr.text, arr[i+1].text]);
                }
                return acc;
            ...
            const res = await axios.post(`${API_URL}/summarize_conversation`, 
                { history: historyPayload }, 
                { headers: { Authorization: `Bearer ${token}` } }
            );
            
            const summaryMsg = { role: "assistant", text: `**Conversation Summary:**\n\n${res.data.summary}`};
            setChat(prev => [...prev, summaryMsg]);

        } catch (error) {
            console.error("Failed to summarize conversation:", error);
            setToast({ show: true, message: `Could not summarize: ${error.response?.data?.detail || error.message}`, type: 'error' });
        } finally {
...
                                <button onClick={backToConversations} className="text-sm font-semibold text-indigo-600 hover:underline">&larr; Back to all conversations</button>
                            )}
                        </div>
                        {activeConversationId && chat.length > 1 && (
                            <button 
                                onClick={handleSummarize} 
                                disabled={isSummarizing}
                                className="flex items-center text-sm font-semibold text-indigo-600 hover:underline disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isSummarizing ? <SpinnerIcon /> : <SummarizeIcon />}
                                <span className="ml-1">{isSummarizing ? 'Summarizing...' : 'Summarize'}</span>
                            </button>
                        )}
                    </div>
