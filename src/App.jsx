...
                        setUserData(data);
                        
                        // Check if user has a workspace ID and it's not empty/null
...
                                                name: data.fullName || firebaseUser.email
                                            }).then(() => {
                                                console.log("Created member document for user");
                                            });
                                        }
                                    });
                                } else {
                                    // Workspace doesn't exist, clear it from user data
                                    setWorkspace(null);
                                    setUserRole('viewer');
                                    updateDoc(userDocRef, { workspaceId: null });
                                }
                            });
                        } else {
                            setWorkspace(null);
...
        } else if (!userData?.workspaceId || userData.workspaceId.trim() === "") {
...
    const { user, userData, setPage } = useApp();
...
            
            await batch.commit();
            setPage('home'); // Redirect to home after successful creation
        } catch (err) {
            console.error(err);
...
                    fullName: memberData.name || 'Unknown User',
...
                                <div key={conv.id} onClick={() => selectConversation(conv)} className="bg-white p-4 rounded-lg shadow-md hover:shadow-xl cursor-pointer transition-shadow flex items-center gap-4">
                                    <ChatBubbleIcon />
                                    <div>
...
                               </button>
                           </form>
                            {userData?.version !== 'pro' && <p className="text-right mt-2 text-sm text-indigo-600">Credits Remaining: {userData?.credits}</p>}
                         </div>
                </div>
            </main>
        </div>
...
        <button onClick={() => { setPage(page); setIsMenuOpen(false); setIsProfileOpen(false); }} className={`font-semibold text-slate-600 hover:text-indigo-600 w-full text-left py-2 md:w-auto md:text-center md:py-0 ${className}`}>
            {children}
        </button>
    );
...
                </div>
                <nav className="hidden md:flex items-center space-x-6">
                    <NavLink page="home">Dashboard</NavLink>
                ...
                <div className="hidden md:flex items-center gap-4 relative">
                    ...
                    {isProfileOpen && (
                        <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 ring-1 ring-black ring-opacity-5">
                            ...
                            <NavLink page="profile" className="px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 block">Profile</NavLink>
...
                        <div className="border-t my-2"></div>
                        <button onClick={handleLogout} className="w-full text-left font-semibold text-red-600 py-2">Logout</button>
                    </nav>
                </div>
            )}
        </header>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                    <div>
                        ...
                        <ul className="mt-4 space-y-4">
                            <li><button onClick={() => setPage('home')} className="text-base text-slate-600 hover:text-slate-900">Home</button></li>
                            <li><button onClick={() => setPage('pricing')} className="text-base text-slate-600 hover:text-slate-900">Pricing</button></li>
                            <li><button onClick={() => setPage('faq')} className="text-base text-slate-600 hover:text-slate-900">FAQ</button></li>
                        </ul>
                    </div>
                    <div>
                        <h3 className="text-sm font-semibold text-slate-500 tracking-wider uppercase">Company</h3>
                        <ul className="mt-4 space-y-4">
                            <li><button onClick={() => setPage('contact')} className="text-base text-slate-600 hover:text-slate-900">Contact</button></li>
                            <li><button onClick={() => setPage('blog')} className="text-base text-slate-600 hover:text-slate-900">Blog</button></li>
                        </ul>
                    </div>
                     <div>
                        <h3 className="text-sm font-semibold text-slate-500 tracking-wider uppercase">Legal</h3>
                        <ul className="mt-4 space-y-4">
                            <li><button onClick={() => setPage('privacy')} className="text-base text-slate-600 hover:text-slate-900">Privacy Policy</button></li>
                            <li><button onClick={() => setPage('terms')} className="text-base text-slate-600 hover:text-slate-900">Terms of Service</button></li>
                        </ul>
                    </div>
                </div>
...
const BlogPage = () => <GenericPage title="Blog" content="Our latest articles, news, and insights will appear here soon. Stay tuned!" />;
const FAQPage = () => <GenericPage title="Frequently Asked Questions" content="Have questions? We're putting together a list of answers to the most common ones. Check back soon!" />;
const ContactPage = () => <GenericPage title="Contact Us" content="You'll be able to reach us through a contact form here shortly." />;
const PrivacyPolicyPage = () => <GenericPage title="Privacy Policy" content="Our full privacy policy document will be available on this page." />;
const TermsOfServicePage = () => <GenericPage title="Terms of Service" content="Our terms of service document will be available on this page." />;
const PillarPage = () => <GenericPage title="AI Document Analysis Guide" content="This is a placeholder for the AI Document Analysis Guide pillar page." />;
const PricingPage = () => <GenericPage title="Pricing" content="Our pricing plans will be detailed here soon. We're working on packages that fit every need." />;
const AdminPage = () => <GenericPage title="Admin Dashboard" content="This is the admin dashboard, accessible only to users with the 'admin' role." />;
